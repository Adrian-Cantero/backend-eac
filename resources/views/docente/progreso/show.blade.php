...
@section('panel')
    <div class="space-y-6">
        ...
        <div>
            <a href="{{ route('docente.dashboard') }}" class="text-sm text-gray-400 hover:text-vfds-primary">← Mi docencia</a>
                            ...
                            @foreach ($ecosistema->situacionesCompetencia->sortBy('codigo') as $sc)
                                <td class="py-3 px-2 text-center">
                                    @if (in_array($sc->id, $conquistadasIds))
                                        @php
                                            $g = $perfil->situacionesConquistadas->firstWhere('id', $sc->id)->pivot
                                                ->gradiente_autonomia;
                                        @endphp
